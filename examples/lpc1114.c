...
static SERVICE_API sim_service_api;
    ...
    ARM_EMULATOR_STATE* State)
{
    ...
    if (FunctionAddress == (uint32_t)(uintptr_t)sim_service_api.GetUptime) {
    ...
    if (FunctionAddress == (uint32_t)(uintptr_t)sim_service_api.DebugWriteLine) {
    ...
    if (FunctionAddress == (uint32_t)(uintptr_t)sim_service_api.DebugWriteLineHex32) {
    ...
    const uint8_t Count)
{
    ...
    if (Address >= SERVICE_API_ADDRESS &&
        Address + Count <= SERVICE_API_ADDRESS + sizeof(SERVICE_API)) {
...
{
    sim_service_api.VersionMajor = 1;
    sim_service_api.VersionMinor = 0;
    sim_service_api.FunctionCount = 7;
...
int main(void)
{
     ...
     *   +0: VersionMajor (1 byte)
     *   +1: VersionMinor (1 byte)
     *   +2: FunctionCount (2 bytes)
    ...
    arm_emulator_reset(
        ...
        (uint8_t*)&sim_service_api, SERVICE_API_ADDRESS, sizeof(SERVICE_API)
    );

    printf("Calling plugin entry point...\n\n");
    arm_emulator_start_function_call((void*)(PROGRAM_BASE + 1), NULL, 0);
